...
9. **步数这一半尚未处理**:第二轮CP在47Å×45Å长胞里rms(c)平台在0.05附近磨~100步(muref、Fast重试均如此),是默认混合(AMIX=0.4/BMIX=1.0/AMIN=0.1)下的电荷晃动;候选是VASP对slab的推荐AMIX=0.2、BMIX=0.0001、AMIN=0.01,用户2026-09-26决定暂不测。
10. **Step-16x1两个电位用新配置重跑(2026-09-26,job 20916398/20916399),两个都正常收敛,没有再出现不收敛**。`Step-16x1_muref_fastcfg`:43分11秒,149步SCF(三轮CP各40/76/33步,每轮都到EDIFF=1e-7,远未碰NELM=200),每步均值15.7 s(8.7~25.6);N_e 792.038710,μ_e −4.907016(目标±8e-5)。`Step-16x1_dUp02_fastcfg`:40分22秒,144步(40/71/33),每步15.2 s;N_e 791.862697,μ_e −5.107525(目标±4e-4,在FERMICONVERGE=0.01内)。与Accurate参考`Step-16x1_muref`(212步,4h00m)对照:N_e Δ3.3e-4 e,TOTEN −220.61924 vs −220.64150(Δ22 meV=0.31 meV/原子,与8x1的9 meV偏移同源、同PREC网格所致),体相n_anion/n_bulk 0.9840 vs 0.9836,近边/台面K_D 0.9838/0.9720 vs 0.9834/0.9718(Δ≤4e-4)。墙钟5.6×。**第6条的"错误轨迹"没有复现**:两次运行第一轮均在第40步收敛到与参考一致的能量(−220.92/−219.93 eV)。第9条的第二轮晃动仍在(muref第二轮76步中48步rms(c)>0.03,第66步才降到<1e-3;dUp02:71步中39步,第52步),只是每步便宜了所以能扛过去——它是速度问题不是收敦问题,这次没有形成不收敛,故未再深查;若以后更长胞再出现超时,先看这一条。
